  ...
  <v-list density="comfortable" class="scrollable-list">
    <v-list-item rounded="pill" ripple v-for="contact in contacts" :key="contact.id" :value="contact" :active="false">
      ...
      <template v-slot:append>
        <v-icon :icon="contact.isFavorite ? 'mdi-star' : 'mdi-star-outline'" class="mr-2"
          @click.stop="toggleFavorite(contact.id)" color="star" />
        <v-icon icon="mdi-trash-can-outline" class="delete-icon" @click.stop="deleteContact(contact.id)" />
      </template>
    </v-list-item>
  </v-list>
</template>
